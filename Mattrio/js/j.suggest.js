	...
	(function($) {
		$.suggest = function(input, options) {
			...
			if($.trim($input.val())=='' || $.trim($input.val())=='') $input.val('');
			...
			function displayItems(items) {
				...
				if (items=='') {//热门城市遍历
					for(h in options.hot_list){
						html+='<li rel="'+options.hot_list[h][0]+'"><a href="#'+h+'"><span>'+options.hot_list[h][2].substring(0,1)+'</span>'+options.hot_list[h][1]+'</a></li>';
					}
					html='<div class="gray ac_result_tip gray_color" style="color:red;">可拼音搜索例如：bm(宝马)</div><ul>'+html+'</ul>';
				}
				else {
					/*if (!items)
					return;
					...
					for (var i = 0; i < options.source.length; i++) {//国内城市匹配
						...
						if (reg.test(options.source[i][0]) || reg.test(options.source[i][1]) || reg.test(options.source[i][2]) || reg.test(options.source[i][3])) {
							html += '<li rel="' + options.source[i][0] + '"><a href="#' + i + '"><span>' + options.source[i][2].substring(0,1) + '</span>' + options.source[i][1] + '</a></li>';
						}
					}
					if (html == '') {
		...
		/*$(document).on("click","#suggest ul li",function(){
			console.log(1)
		})*/
